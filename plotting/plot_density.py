...
def plot_encoding_embedding(enc_distances, emb_distances,
                            sample_subpopulations,
                            sub_to_super,
                            colors,
                            chrm,
    ...
    for superpop in superpops:
        ...
        for sampleA in samples:
            for sampleB in samples:
                ...
                if sampleA_0 in enc_distances and sampleB_0 in enc_distances[sampleA_0]:
                    encoding_distances.append(enc_distances[sampleA_0][sampleB_0])
                    embedding_distances.append(emb_distances[sampleA_0][sampleB_0])
                if sampleA_0 in enc_distances and sampleB_1 in enc_distances[sampleA_0]:
                    encoding_distances.append(enc_distances[sampleA_0][sampleB_1])
                    embedding_distances.append(emb_distances[sampleA_0][sampleB_1])
                if sampleA_1 in enc_distances and sampleB_0 in enc_distances[sampleA_1]:
                    encoding_distances.append(enc_distances[sampleA_1][sampleB_0])
                    embedding_distances.append(emb_distances[sampleA_1][sampleB_0])
                if sampleA_1 in enc_distances and sampleB_1 in enc_distances[sampleA_1]:
                    encoding_distances.append(enc_distances[sampleA_1][sampleB_1])
                    embedding_distances.append(emb_distances[sampleA_1][sampleB_1])
    ...
    fig.suptitle('Encoding vs. Embedding Distances\nChromosome ' + chrm, fontsize=20, fontweight='bold')
    fig_name = out + 'encoding_embedding_chrm' + chrm + '.png'
    plt.tight_layout()
    plt.savefig(fig_name)
    ...
    #
    # # plot encoding vs. embedding distances
    # fig, ax = plt.subplots(figsize=(10, 10), dpi=300)
    # ax.scatter(encoding_distances, embedding_distances, alpha=0.5)
    # ax.set_xlabel('Encoding Distance')
    # ax.set_ylabel('Embedding Distance')
    #
    # ax.spines['top'].set_visible(False)
    ...
    # # calculate r^2
    # r2 = stats.pearsonr(encoding_distances, embedding_distances)[0] ** 2
    # # add r^2 to plot with decimal places = 2
    # ax.text(0.1, 0.9, 'r^2 = ' + str(round(r2, 2)),
    #         horizontalalignment='center',
    #         verticalalignment='center',
    #         transform=ax.transAxes,
    #         fontsize=15,
    #         fontweight='bold')
    #
    # title = ('Encoding vs. Embedding Distances'
    #             '\nChromosome ' + chrm)
    # fig.suptitle(title, fontsize=20, fontweight='bold')
    # fig_name = out + 'encoding_embedding_chrm' + chrm + '.png'
    # plt.tight_layout()
...
def main():
    ...
    # read density files
    chroms = [str(i) for i in range(8, 10)]
    ...
    for chrm in chroms:
        ...
        for sample, densities in sample_densities.items():
            ...
            sample_densities[sample] = [densities[9]]
        ...
        # # print('Plotting density by r^2...', chrm)
        # plot_encoding_embedding(enc_distances, emb_distances,
        #                         sample_subpopulations,
        #                         sub_to_super,
        #                         colors,
        #                         chrm,
