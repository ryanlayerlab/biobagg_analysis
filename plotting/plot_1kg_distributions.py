...
def plot_trio_all_populations(trio_scores, out_dir):
    ...
    for i, category in enumerate(categories_labels):
        ...
        # plot density plot
        sns.kdeplot(scores, color=trio_color_dict[category], ax=axs, linewidth=2, fill=True, alpha=0.3)
    ...
    # axs legend with title, labels, and colors, no border
    axs.legend(['1', 'unrelated'], loc='upper right', frameon=False, title='Number of Meoisis')
